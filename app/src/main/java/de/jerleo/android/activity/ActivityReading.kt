...
class ActivityReading : FragmentActivity(), View.OnClickListener, OnDateChangedListener {
    ...
    private fun newReading() {
        reading = Reading()

        val bundle = this.intent.extras
        bundle?.let {
            val id = it.getLong(Constants.METER)

            val meter = Home.instance.meter(id)
            meter?.let {
                this.meter = it
                title = getString(R.string.new_reading) + ": " + it.number
                meterUnit.text = it.unit.toString()
